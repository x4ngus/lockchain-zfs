...
verify_checksum() {
    ...
    if (( size != 32 )); then
        fallback_exit "Key file $KEY_PATH must be 32 raw bytes (found $size). Replay doctor/init to convert."
    fi
...
main() {
    ...
    if zfs load-key -a; then
        ...
        local rc=$?
        fallback_exit "zfs load-key -a returned non-zero status (${rc}); native prompts will take over."
    fi
}
